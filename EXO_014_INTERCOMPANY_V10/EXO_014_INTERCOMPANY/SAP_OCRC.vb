...
#Region "Inicialización"
    ...
    Public Overrides Function filtros() As SAPbouiCOM.EventFilters
        Dim fXML As String = objGlobal.Functions.leerEmbebido(Me.GetType(), "Filtros_OCRC.xml")
                        ...
                        Select Case infoEvento.EventType
                            ...
                            Case SAPbouiCOM.BoEventTypes.et_FORM_DATA_UPDATE
                                If infoEvento.ActionSuccess Then
                                    If GuardarInterCoOCRC("5", "OCRC") = False Then
                                        Return False
                                    End If
                                End If
...
#Region "Métodos auxiliares"
    ...
    Private Function GuardarInterCoOCRC(ByVal sTableCategory As String, ByVal sTableName As String) As Boolean
        ...
        Try
            If EXO_GLOBALES.EmpresaConectadaEsMatriz(objGlobal) = True Then
                ...
                If oRs.RecordCount > 0 Then
                    oRsAux2.DoQuery("DELETE FROM [INTERCOMPANY].dbo.[REPLICATE] WHERE tableName = '" & sTableName & "' AND dateAdd = '" & Now.Year & "-" & Right("0" & Now.Month.ToString, 2) & "-" & Right("0" & Now.Day.ToString, 2) & "'")
                    oRsAux2.DoQuery("DELETE FROM [INTERCOMPANY].dbo.[REPLICATE] WHERE tableName = '" & sTableName & "' AND codeTable NOT IN (SELECT CreditCard " & _
                                                                                                                                            "FROM " & sTableName & " WITH (NOLOCK))")
                    For i As Integer = 0 To oNodes.Count - 1
                        oNode = oNodes.Item(i)

                        oRsAux.DoQuery("SELECT CreditCard AS Codigo, CardName AS Codigo2 " & _
                                       "FROM " & sTableName & " WITH (NOLOCK) " & _
                                       "WHERE UpdateDate = '" & Now.Year & "-" & Right("0" & Now.Month.ToString, 2) & "-" & Right("0" & Now.Day.ToString, 2) & "' " & _
