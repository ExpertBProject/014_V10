...
#Region "Inicialización"
    ...
    Public Overrides Function filtros() As SAPbouiCOM.EventFilters
        Dim fXML As String = objGlobal.funciones.leerEmbebido(Me.GetType(), "Filtros_OCRC.xml")
        ...
        Try
            If EXO_GLOBALES.EmpresaConectadaEsMatriz(objglobal) = True Then
                ...
                If oRs.RecordCount > 0 Then
                    ...
                    oRsAux2.DoQuery("DELETE FROM [INTERCOMPANY].dbo.[REPLICATE] WHERE tableName = '" & sTableName & "' AND codeTable NOT IN (SELECT CreditCard " &
                                                                                                                                            "FROM " & sTableName & " WITH (NOLOCK))")
                    For i As Integer = 0 To oNodes.Count - 1
                        oNode = oNodes.Item(i)

                        oRsAux.DoQuery("SELECT CreditCard AS Codigo, CardName AS Codigo2 " &
                                       "FROM " & sTableName & " WITH (NOLOCK) " &
                                       "WHERE UpdateDate = '" & Now.Year & "-" & Right("0" & Now.Month.ToString, 2) & "-" & Right("0" & Now.Day.ToString, 2) & "' " &
